...
local function ConsolidateStacks(bagIDs, pinnedSlots)
		...
		if numSlots and numSlots > 0 then
			for slot = 1, numSlots do
				...
				if link then
					...
					local _, _, itemRarity = Guda.GetItemInfo(itemID)
	...
	for _, group in pairs(itemGroups) do
		if table.getn(group.stacks) > 1 then
			...
			-- pos 8 = stackCount in canonical/modern order
		...
		if addon.Modules.Utils:IsBagValid(bagID) then
			for slot = 1, numSlots do
				...
				if itemLink then
					...
					local itemID = GetItemID(itemLink)
					local name, link, quality, iLevel, minLevel, itemType, subType, stackCount, equipLoc, iconTex, sellPrice = Guda.GetItemInfo(itemID)
