...
function Utils:SafeCall(moduleName, methodName, ...)
    ...
    -- Use Lua 5.1 native varargs; the legacy `arg` table is not reliably
    -- populated on Ascension Epoch's Lua environment.
...
-- Canonical field order (matches WotLK 3.3.5a native, normalized from vanilla via
-- Guda.GetItemInfo in ClientCompat.lua):
...
function Utils:GetItemInfoSafe(itemID)
    ...
    local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = Guda.GetItemInfo(itemID)
...
function Utils:GetItemInfo(itemLink)
    ...
    local itemName, retLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = self:GetItemInfoSafe(itemID)

    if itemName then
        itemCache[itemLink] = {itemName, retLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice}
        return itemName, retLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice
    end
...
function Utils:GetInboxItemLink(index, itemIndex)
    ...
    if name then
        local itemName, link = Guda.GetItemInfo(name)
        if link then
            return link
        end
    end
...
function Utils:IsQuestItem(bagID, slotID, itemData, isOtherChar, isBank)
    ...
    if itemLink then
        ...
        if itemID then
            -- pos 6 = itemType in canonical/modern order; also mirror to
            -- itemCategory for the locale-dependent "Weapon"/"Armor"/"Quest"
            -- checks below (pos 5 on modern is minLevel, not a string).
            local _, _, _, _, _, typ = self:GetItemInfoSafe(itemID)
            itemCategory = typ or itemCategory
...
function Utils:GetSpecializedBagType(bagID)
    ...
    -- Only player bags (1-4) and bank bags (5-10) map to inventory slots.
    -- Ascension Epoch sometimes fires BAG_UPDATE with IDs outside that range,
    -- and ContainerIDToInventoryID raises a hard error on invalid IDs
    -- instead of returning nil, so gate it on a numeric range.
    if type(bagID) ~= "number" or bagID < 1 or bagID > 11 then
        return nil
    end
...
function Utils:GetItemPreferredContainer(itemLink)
    ...
    local itemName, _, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = self:GetItemInfoSafe(itemID)
    if not itemType then return nil end

    -- Locale-independent projectile routing. Don't gate on itemType string
...
function Utils:IsEquipment(itemLink)
	...
	-- Fallback: check type (in case itemEquipLoc is not set)
	if itemType then
		local typeLower = string.lower(itemType)
		if typeLower == "armor" or typeLower == "weapon" then
			return true
		end
	end
...
function Utils:IsHerbItem(itemLink)
    ...
    local name, _, quality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = self:GetItemInfoSafe(itemID)
