...
function Main:Initialize()
    ...
    addon.Modules.Events:OnPlayerLogin(function()
        ...
        -- Surface silent Lua errors so regressions are visible during the port shakedown.
        if SetCVar then pcall(SetCVar, "scriptErrors", "1") end
...
function Main:SetupSlashCommands()
    ...
    SlashCmdList["Guda"] = function(msg)
        ...
        elseif msg == "diag" then
            -- Runtime diagnostic: dumps bag/pool/container state
            local function yn(v) return v and "yes" or "no" end
            addon:Print("--- Guda diag ---")
            -- Frame / container
            local frame = getglobal("Guda_BagFrame")
            local container = getglobal("Guda_BagFrame_ItemContainer")
            addon:Print("BagFrame exists=%s shown=%s", yn(frame), yn(frame and frame:IsShown()))
            if frame then
                addon:Print("  size=%dx%d", frame:GetWidth() or 0, frame:GetHeight() or 0)
            end
            addon:Print("ItemContainer exists=%s", yn(container))
            if container then
                addon:Print("  size=%dx%d shown=%s", container:GetWidth() or 0, container:GetHeight() or 0, yn(container:IsShown()))
            end
            -- Pool
            if Guda_GetButtonPoolStats then
                local s = Guda_GetButtonPoolStats()
                addon:Print("Pool: total=%d shown=%d hidden=%d inUse=%d avail=%d max=%d",
                    s.total, s.shown, s.hidden, s.inUse, s.available, s.maxSize)
            else
                addon:Print("Pool: Guda_GetButtonPoolStats missing")
            end
            -- Scanner
            local bs = addon.Modules and addon.Modules.BagScanner
            addon:Print("BagScanner loaded=%s", yn(bs))
            if bs and bs.GetBagData then
                local ok, data = pcall(function() return bs:GetBagData() end)
                if not ok then
                    addon:Print("  GetBagData ERRORED: %s", tostring(data))
                else
                    local bags, items = 0, 0
                    for bagID, bag in pairs(data or {}) do
                        bags = bags + 1
                        if bag and bag.slots then
                            for _, it in pairs(bag.slots) do if it then items = items + 1 end end
                        end
                    end
                    addon:Print("  bagData: %d bags, %d items", bags, items)
                end
            end
            -- Slot counts from Blizzard API
            local util = addon.Modules and addon.Modules.Utils
            if util and util.GetBagSlotCount then
                local total = 0
                for _, bid in ipairs({0,1,2,3,4}) do
                    local n = util:GetBagSlotCount(bid) or 0
                    total = total + n
                    addon:Print("  bag %d: %d slots", bid, n)
                end
                addon:Print("  total player bag slots: %d", total)
            end
            -- Try Update in pcall
            if addon.Modules and addon.Modules.BagFrame and addon.Modules.BagFrame.Update then
                local ok, err = pcall(function() addon.Modules.BagFrame:Update() end)
                if ok then
                    addon:Print("BagFrame:Update() ran without error")
                else
                    addon:Print("BagFrame:Update() ERRORED: %s", tostring(err))
                end
            end
            -- Re-check pool after update
            if Guda_GetButtonPoolStats then
                local s = Guda_GetButtonPoolStats()
                addon:Print("Pool after Update: total=%d shown=%d", s.total, s.shown)
            end
            -- ToggleBackpack hooked?
            addon:Print("ToggleBackpack exists=%s", yn(ToggleBackpack))
            addon:Print("--- end diag ---")
