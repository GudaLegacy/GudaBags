...
-- Bank bag range is client-dependent, resolved at load time via ClientCompat:
--   Vanilla Blizzard 1.12 : 6 bags (5..10)
--   TurtleWoW 1.12        : 7 bags (5..11)
--   WotLK 3.3.5a          : 7 bags (5..11)
local bankBagCount = (addon.GetBankBagCount and addon.GetBankBagCount()) or 6
C.BANK_LAST = C.BANK_FIRST + bankBagCount - 1

-- Bag ID ranges for iteration
C.BAG_IDS = {0, 1, 2, 3, 4}
C.BANK_BAG_IDS = {}
for i = C.BANK_FIRST, C.BANK_LAST do
    table.insert(C.BANK_BAG_IDS, i)
end
C.ALL_BAG_IDS = {0, 1, 2, 3, 4, -2}  -- Including keyring

-- Rebuild the Init.lua bag lists to include the 7th bank bag on TBC+/TurtleWoW.
-- BANK_BAGS includes BANK_CONTAINER (-1). ALL_BAGS = bags + bank_bags.
C.BANK_BAGS = { C.BANK_CONTAINER }
for i = C.BANK_FIRST, C.BANK_LAST do
    table.insert(C.BANK_BAGS, i)
end
C.BAG_6 = nil  -- legacy placeholder; real slot exposed via BANK_BAG_7 when present
if bankBagCount >= 7 then
    C.BANK_BAG_7 = 11
end
C.ALL_BAGS = { 0, 1, 2, 3, 4, C.BANK_CONTAINER }
for i = C.BANK_FIRST, C.BANK_LAST do
    table.insert(C.ALL_BAGS, i)
end
...
-- Bank Slots
--=============================================================================
-- Vanilla 1.12: 24. WotLK 3.3.5: 28. Resolved from NUM_BANKGENERIC_SLOTS at load.
C.BANK_MAIN_SLOTS = (addon.GetBankMainSlotCount and addon.GetBankMainSlotCount()) or 24
