...
local function GetCursorItemCategory()
    ...
    if info.link then
        itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = Guda.GetItemInfo(info.link)
    end
...
local function GetOrCreateIndicator()
    ...
    plus:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\plus")
    f.plus = plus
...
local function AcquireLockIcon()
	...
	if not icon then
		...
		shadow:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\lock_glow")
		...
		local texture = icon:CreateTexture(nil, "OVERLAY")
		texture:SetAllPoints(icon)
		texture:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\lock_glow")
		icon.texture = texture
	end
	return icon
end
...
local function AcquirePinIcon()
	...
	if not icon then
		...
		shadow:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\pin")
		...
		local texture = icon:CreateTexture(nil, "OVERLAY")
		texture:SetAllPoints(icon)
		texture:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\pin")
		icon.texture = texture
	end
	return icon
end
...
local function GetItemDisplayInfo(bagID, slotID, itemData, isReadOnly)
    ...
    if not isReadOnly then
        ...
        elseif bagID == -2 then
            ...
            if link then
                hasItem = true
                -- pos 10 = texture in canonical/modern order
                local _, _, _, _, _, _, _, _, _, itemTexture = Guda.GetItemInfo(link)
...
function Guda_ItemButton_SetItem(self, bagID, slotID, itemData, isBank, otherCharName, matchesFilter, isReadOnly)
    ...
    if not self.isReadOnly and bagID and slotID and self.hasItem then
        ...
        if itemLink then
            local itemName, _, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = Guda.GetItemInfo(itemLink)
...
function Guda_ItemButton_OnEnter(self)
	...
	if self.bagID and self.slotID and not self.otherChar and not self.isReadOnly then
		...
		if link then
			...
			if itemLink then
				local itemName = Guda.GetItemInfo(itemLink)
	...
	if addon.DEBUG and self.hasItem and self.bagID and self.slotID and not self._debugPrinted then
		...
		if link then
			local itemID = addon.Modules.Utils:ExtractItemID(link)
			if itemID then
				local itemName, _, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType = Guda.GetItemInfo(itemID)
				addon:Debug("Item: %s (ID: %s)", tostring(itemName), tostring(itemID))
				addon:Debug("  Type: %s | SubType: %s | MinLvl: %s", tostring(itemType), tostring(itemSubType), tostring(itemMinLevel))
