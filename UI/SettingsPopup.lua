...
function Guda_SettingsPopup_OnShow(self)
    ...
    if bagViewDropdown then
        ...
        UIDropDownMenu_SetText(bagViewDropdown, bagViewType == "single" and "Single" or "Category")
    end

    if bankViewDropdown then
        UIDropDownMenu_SetSelectedValue(bankViewDropdown, bankViewType)
        UIDropDownMenu_SetText(bankViewDropdown, bankViewType == "single" and "Single" or "Category")
    end
...
local function Guda_BagViewDropdown_Initialize()
    ...
    for _, option in ipairs(bagViewOptions) do
        ...
        info.func = function()
            ...
            UIDropDownMenu_SetText(getglobal("Guda_SettingsPopup_BagViewDropdown"), val == "single" and "Single" or "Category")
...
function Guda_SettingsPopup_BagViewDropdown_OnLoad(self)
    ...
    UIDropDownMenu_SetText(self, current == "single" and "Single" or "Category")
...
local function Guda_BankViewDropdown_Initialize()
    ...
    for _, option in ipairs(bankViewOptions) do
        ...
        info.func = function()
            ...
            UIDropDownMenu_SetText(getglobal("Guda_SettingsPopup_BankViewDropdown"), val == "single" and "Single" or "Category")
...
local MARK_ICONS = {
    "Interface\\AddOns\\GudaBags\\Assets\\equipment",
    "Interface\\AddOns\\GudaBags\\Assets\\plus",
    "Interface\\AddOns\\GudaBags\\Assets\\fav",
    "Interface\\AddOns\\GudaBags\\Assets\\combat",
    "Interface\\AddOns\\GudaBags\\Assets\\Cog",
    "Interface\\AddOns\\GudaBags\\Assets\\guild",
}
...
local function GetRuleRowFrame(index)
    ...
    reqTex:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\pin")
    reqBtn.tex = reqTex
    ...
    dropZone:SetScript("OnReceiveDrag", function()
        ...
        if CursorHasItem and CursorHasItem() then
            ...
            if info and info.itemID then
                ...
                -- Update icon
                local _, _, _, _, _, _, _, _, _, texture = Guda.GetItemInfo(info.link or info.itemID)
                if texture then
                ...
                if row.dropZone then
                    if isItemID then
                        ...
                        if itemID and itemID > 0 then
                            local _, _, _, _, _, _, _, _, _, texture = Guda.GetItemInfo(itemID)
                            if texture then
