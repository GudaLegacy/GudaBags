...
function BankFrame:UpdateSingleSlot(bagID, slotID, passedButton)
    ...
    if itemLink then
        ...
        if itemID then
            local name, link, itemQuality, iLevel, _, itemType, subType, stackCount, equipLoc = Guda.GetItemInfo(itemID)
            itemData = {
                link = itemLink,
                texture = texture,
...
function BankFrame:CreateGudaButtonOnBlizzardUI()
    ...
    texture:SetTexture("Interface\\AddOns\\GudaBags\\Assets\\Chest")
